...
const appliedFilters = ref({ search: "", type: "", direction: "", from: "", to: "" });
...
const applyFilters = async () => {
  if (filters.value.from && filters.value.to && filters.value.from > filters.value.to) {
    errorMessage.value = "The From date cannot be later than the To date.";
    return;
  }
  isFiltering.value = true;
  appliedFilters.value = { ...filters.value };
  try { await load(); } finally { isFiltering.value = false; }
};
const clearFilters = async () => {
  filters.value = { search: "", type: "", direction: "", from: "", to: "" };
  appliedFilters.value = { ...filters.value };
  await load();
};
...
<template>
  <DashboardLayout page-title="Transactions" page-subtitle="Financial audit trail">
    ...
    <section class="card panel-card"><div class="card-body"><div class="ledger-head"><div><h5 class="mb-1">Audit trail</h5><p class="text-muted mb-0">Ledger entries cannot be edited or deleted.</p></div><span class="immutable"><i class="bi bi-lock"></i>Append only</span></div>
      <form class="filter-grid" @submit.prevent="applyFilters"><label class="search-field"><i class="bi bi-search"></i><input v-model.trim="filters.search" placeholder="Member, reference or description" /></label><select v-model="filters.type"><option value="">All movement types</option><option v-for="type in types" :key="type[0]" :value="type[0]">{{ type[1] }}</option></select><select v-model="filters.direction"><option value="">Money in and out</option><option value="inflow">Money in</option><option value="outflow">Money out</option></select><input v-model="filters.from" type="date" aria-label="From date" /><input v-model="filters.to" type="date" aria-label="To date" /><div class="filter-actions"><button v-if="activeFilterCount" type="button" class="btn btn-outline-secondary" @click="clearFilters">Clear</button><button type="submit" class="btn btn-primary" :disabled="isFiltering"><span v-if="isFiltering" class="spinner-border spinner-border-sm"></span>{{ isFiltering ? 'Applying...' : 'Apply filters' }}</button></div></form>
      <div v-if="activeFilterCount" class="filter-feedback"><i class="bi bi-funnel"></i>{{ activeFilterCount }} filter{{ activeFilterCount === 1 ? '' : 's' }} applied Â· {{ transactions.length }} result{{ transactions.length === 1 ? '' : 's' }}</div>
